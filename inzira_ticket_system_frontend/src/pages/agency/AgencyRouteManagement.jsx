...
const AgencyRouteManagement = () => {
  ...
  const handleUpdateAgencyRoute = async (e) => {
    e.preventDefault()
    try {
      await agencyAPI.updateAgencyRoute(editingRoute.id, {
        ...routeForm,
        agencyId: user.roleEntityId,
        price: parseFloat(routeForm.price)
      })
      toast.success('Agency route updated successfully')
      resetForm()
      fetchAgencyRoutes()
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to update agency route')
    }
  }
  ...
  const startEditRoute = (agencyRoute) => {
    setEditingRoute(agencyRoute)
    setRouteForm({
      agencyId: user.roleEntityId,
      routeId: agencyRoute.route.id.toString(),
      price: agencyRoute.price.toString(),
      pickupPointIds: agencyRoute.pickupPoints.map(p => p.id),
      dropPointIds: agencyRoute.dropPoints.map(p => p.id)
    })
    // Fetch route points for the selected route
    handleRouteChange(agencyRoute.route.id.toString())
    setShowForm(true)
  }
              ...
              {agencyRoutes.map((agencyRoute) => (
                <div key={agencyRoute.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex justify-end">
                    <button
                      onClick={() => startEditRoute(agencyRoute)}
                      className="mr-2 px-3 py-2 text-blue-600 hover:bg-blue-50 rounded-md"
                    >
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">
                {editingRoute ? 'Edit Agency Route' : 'Add New Agency Route'}
              </h3>
              <button
                onClick={resetForm}
            ...
            <form onSubmit={editingRoute ? handleUpdateAgencyRoute : handleCreateAgencyRoute}>
